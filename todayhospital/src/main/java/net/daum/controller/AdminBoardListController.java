...
/* 관리자 단 게시판 목록보기 컨트롤러 */
public class AdminBoardListController implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		AdminBoardService adminBoardService = new AdminBoardServiceImpl();//부모 인터페이스 타입으로 업캐스팅
		...
		if(admin_id==null) {
			...
			int listcount=adminBoardService.getListCount(findB);
			...
			List<BoardDTO> blist=adminBoardService.getBoardList(findB);
			//목록
			...
			request.setAttribute("blist",blist);//blist 키이름에 검색전,검색후 레코드 목록 저장
			...
			forward.setPath("./WEB-INF/views/admin/admin_board_list.jsp");////뷰페이지 폴더경로와 파일명 지정	
			return forward;				
		}
		return null;
	}
