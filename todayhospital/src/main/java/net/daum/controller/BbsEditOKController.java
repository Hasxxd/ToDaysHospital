...
public class BbsEditOKController implements Action {
	
	/* 한개파일/다중파일 수정 업로드 컨트롤러 */	
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		BbsDTO db_pwd=bbsService.getBbsCont2(bbs_no);//오라클로 부터 비번을 가져옴.
		...
		if(!db_pwd.getBbs_pwd().equals(bbs_pwd)) {
			...
			bbsService.editBbs(bbs);//번호를 기준으로 글쓴이,글제목,글내용만 수정
			...
			for (Part part : request.getParts()) {
				...
				if ("bbs_file".equals(part.getName()) && part.getSize() > 0) {//수정 첨부파일이 있는 경우	
					...
					bbsService.updateAttached_file(7,bbs_no);
			...
			forward.setPath("bbs_cont.do?bbs_no="+bbs_no+"&page="+page+"&state=cont");
			return forward;
		}//if else		
